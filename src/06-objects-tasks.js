...
SelectorsBuilder.prototype = {
  checkSequence(index) {
    if (this.unChangeableFildes.indexOf(index) !== -1 && this.currentStage === index) throw new Error('Element, id and pseudo-element should not occur more then one time inside the selector');
  ...
  stringifyItem(items, before, after) {
    let newItems = items || [];
    if (!Array.isArray(items) && items != null) newItems = [items];

    return newItems.reduce((previous, current) => previous + before + current + after, '');
  },
...
const cssSelectorBuilder = {
  element(value) {
    return new SelectorsBuilder().element(value);
  },

  id(value) {
    return new SelectorsBuilder().id(value);
  },

  class(value) {
    return new SelectorsBuilder().class(value);
  },

  attr(value) {
    return new SelectorsBuilder().attr(value);
  },

  pseudoClass(value) {
    return new SelectorsBuilder().pseudoClass(value);
  },

  pseudoElement(value) {
    return new SelectorsBuilder().pseudoElement(value);
  },
